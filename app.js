...
function callApi(typeOfSearch) {
  ...
  let ajaxUrlResolved = ajaxUrl + typeOfSearch + "?api_key=" + apiKey + "&query=" + searchInput.value + "&page=" + pageCount;
...
blank, by default the value of the search field will be the letter "A". And calls the function callApi.
*/
...
function createFavoritesPosters(favorites) {
  ...
  for (let result of favorites) {
    ...
    posterView.innerHTML +=
      `<div class="poster-view__container"><div class="imgBox"><img class="img" src="${poster}" alt="film-poster" />
...
function getFavoriteInArray(results) {
  const getFavoritesBtn = document.querySelectorAll(".container-btn, .container-btn-added");
  for (let i = 0; i < getFavoritesBtn.length; i++) {
    getFavoritesBtn[i].addEventListener('click', function () {
      ...
      const favorite = Array.from(new Set(favorites.map(a => a.id)))
        .map(id => {
